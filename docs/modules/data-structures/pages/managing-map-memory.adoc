...
and therefore map entries, than that of a larger cluster. Thus, for a small cluster, the overhead of removing entries per partition can impact overall performance.
...
the `PER_NODE` attribute for `max-size`, Hazelcast calculates the maximum size for every partition. Hazelcast uses the following equation to calculate the maximum size of a partition:
