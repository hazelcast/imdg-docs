...
. Initialization starts depending on the value of the `initial-mode` property. If it is set to `EAGER`, initialization starts on all partitions as soon as the map is created (i.e. when `getMap` is called).  If it is set to `LAZY`, data is loaded when a `map.get` or other operation tries to read an entry from the map.
