...
To load data from external systems in a map you will use the Java link:{javadocBaseUrl}/javadoc/com/hazelcast/map/MapLoader.html[`MapLoader`] interface. 

To save map entries to an external system, you will use the Java link:{javadocBaseUrl}/javadoc/com/hazelcast/map/MapStore.html[`MapStore`] interface.
...
|LAZY
|After getting or creating the map, the `MapLoader.loadAllKeys()` method is invoked on a partition-by-partition basis. The first time you use any of the `IMap` methods (e.g. `map.get`) to get a value from a partition, the member that owns that partition will load all entries for that partition only. LAZY is the default mode. 
|===

NOTE: If you add indices to your map with the link:{javadocBaseUrl}/javadoc/com/hazelcast/config/IndexConfig.html[IndexConfig^]
...
. Initialization starts depending on the value of the `initial-mode` property. If it is set to `EAGER`, initialization starts on all partitions as soon as the map is created (i.e. when `getMap` is called).  If it is set to `LAZY`, data is loaded partition by partition when a `map.get` or other operation tries to read an entry from the partition.
