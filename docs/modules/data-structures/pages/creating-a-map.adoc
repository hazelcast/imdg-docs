...
(Need client syntax for serialization method for C++, C Sharp, nodejs, Python)
...
func (c Country) FactoryID() (factoryID int32) {
	return 1
}
func (c Country) ClassID() (classID int32) {
	return 1
}
func (c Country) WritePortable(writer serialization.PortableWriter) error {
	writer.WriteString("name", c.Name)
	writer.WriteInt64("population", c.Population)
	return nil
}
func (c *Country) ReadPortable(reader serialization.PortableReader) error {
	c.Name = reader.ReadString("name")
	c.Population = reader.ReadInt64("population")
	return nil
}
type CountryPortableFactory struct {
}
func (c CountryPortableFactory) Create(classID int32) (instance serialization.Portable) {
	if classID == 1 {
		return &Country{}
	}
	panic("factory not found")
}
func (c CountryPortableFactory) FactoryID() int32 {
	return 1
}
// ...
// error handling is omitted for brevity
cb := hazelcast.ConfigBuilder{}
cb.Serialization().AddPortableFactory(&CountryPortableFactory{})
client, _ := hazelcast.StartNewClientWithConfig(cb)
countries, _ := client.GetMap("countries")
countries.Set(1, &Country{"Tokyo", 13_960_236})
countries.Set(2, &Country{"Paris", 2_175_601})
countries.Set(3, &Country{"Washington", 689_545})
countries.Set(4, &Country{"Ankara", 5_663_322})
countries.Set(5, &Country{"Brussels", 1_208_542})
