...
If you are working with an external system of record e.g. a database, you can use the MapLoader feature to automatically push updates and maintain synchronization between in-memory data and the external store. This feature, called MapLoader, is described fully in the xref:working-with-external-data.adoc[Working with External Data] page.
...
distributedMap, _ := hzClient.GetMap("map")
entries := []types.Entry{
	types.NewEntry(1, "John"),
	types.NewEntry(2, "Jack"),
	types.NewEntry(3, "David"),
}
distributedMap.PutAll(entries)
size, _ := distributedMap.Size()
fmt.Println(size)
...
The method `map.replaceAll` is used in this example to modify the entire contents of a map. When you use `map.replaceAll`, all existing entries are affected, but keys with null values are not modified. 
...
|Method
|Erase in-memory
|Erase from external data store
|Returns
...
If you are not working with a external data store (e.g. database) using the MapLoader feature, `map.evict` and `map.delete` have the same effect. This feature is described in detail in the xref:working-with-external-data.adoc[Working With External Data] section of the documentation.  
...
studentmap.removeAll(Predicates.sql("gradYear = 2020"));
...
studentMap.RemoveAll(predicate.Equal("GradYear", 2020))
