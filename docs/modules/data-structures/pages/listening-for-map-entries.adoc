...
NOTE: Listeners are a server-side feature. Because these operations run within the Hazelcast cluster natively, the interfaces that perform these functions are only available in Java.
...
The next code sample modifies the map entry for Smith, which will trigger the listener. 
...
The Modify code triggered the listener, which then printed the system output. You can replace the `System.out.println` with any code that you want to run omly when the specified map event occurs. 
