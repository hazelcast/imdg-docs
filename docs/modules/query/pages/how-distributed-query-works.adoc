...
<hazelcast>
    ...
    <map name="persons">
        <indexes>
            <index>BITMAP(age, uniqueId, RAW)</index>
        </indexes>
    </map>
    ...
</hazelcast>
...
<hazelcast>
    ...
    <map name="persons">
        <indexes>
            <index>BITMAP(age, uniqueId, LONG)</index>
        </indexes>
    </map>
    ...
</hazelcast>
...
a simple bitmap index definition like `BITMAP(age)` is equivalent to the
following full-form definition: `BITMAP(age, __key, OBJECT)`, which
indexes `age` attribute, uses `IMap` entry keys (`__key`) interpreted
...
* `<transform>`: Specifies the transformation to be applied
...
----
BITMAP(name.first)
BITMAP(name.first, __key, RAW)
BITMAP(name.first, __key.id, RAW)
BITMAP(name.first, id.external, RAW)
...
----
