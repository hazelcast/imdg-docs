...
See the xref:distributed-events.adoc#registering-map-listeners[Registering Map Listeners section].
* `in-memory-format`: Type of the data to be stored in your query cache.
See the xref:dds.adoc#setting-in-memory-format[in-memory format section].
...
xref:dds.adoc#configuring-map-eviction[Configuring Map Eviction section].
