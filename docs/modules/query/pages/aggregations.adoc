...
Fast-Aggregations functionality is the successor of the xref:query:aggregators.adoc[Aggregators].
They are equivalent to the MapReduce Aggregators in most of the use cases, but instead
of running on the MapReduce engine they run on the Query infrastructure.
Their performance is tens to hundreds times better since they run in parallel for each
...
https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/aggregation/Aggregator.html[Aggregator Javadoc^]
...
to its https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/map/IMap.html#aggregate-com.hazelcast.aggregation.Aggregator-[Javadoc^]
