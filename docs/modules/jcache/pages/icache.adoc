...
existing one to connect to a Hazelcast cluster. See the following examples
to see the ways on achieving these.
...
`com.domain`, a `CacheManager` can be configured to start a new `HazelcastInstance` by specifying the configuration file location as a property:
