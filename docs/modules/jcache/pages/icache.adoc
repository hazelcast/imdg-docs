...
Note that if the Hazelcast configuration file does specify an instance name, then any ``CacheManager``s referencing the same configuration file locates by name and reuses the same `HazelcastInstance`.
...
See the https://docs.hazelcast.org/docs/{full-version}/javadoc/com/hazelcast/cache/ICache.html[ICache Javadoc^]
