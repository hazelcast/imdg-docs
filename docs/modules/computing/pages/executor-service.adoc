...
* `statistics-enabled`: Specifies whether the statistics gathering is enabled for your Executor Service. If set to `false`, you cannot collect statistics in your implementation (using `getLocalExecutorStats()`) and also {mc-docs}#monitoring-executors[Hazelcast Management Center^] will not show them. Its default value is `true`.
