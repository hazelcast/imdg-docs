...
* `com.hazelcast.core` for `hazelcast-{full-version}.jar` and
`hazelcast-enterprise-{full-version}.jar`

Don't use `hazelcast-all-{full-version}.jar` or
`hazelcast-enterprise-all-{full-version}.jar` on the modulepath as it could
...
_This example expects `hazelcast-{full-version}.jar` placed in the `lib` directory._