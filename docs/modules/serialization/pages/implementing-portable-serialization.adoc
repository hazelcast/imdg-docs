...
https://docs.hazelcast.org/docs/{page-component-version}/javadoc/com/hazelcast/nio/serialization/VersionedPortable.html[VersionedPortable^]
...
Hazelcast lazily creates a class definition of portable internally when the user first serializes.
This class definition is stored and used later for deserializing that portable class.

Firstly, `writePortable` method cannot be called with a `null` object.
`writeNullPortable` should be used instead as shown below:
...
Secondly, when the user tries to serialize a null portable when there is no
class definition at the moment, Hazelcast throws a serialization exception saying "Cannot write null portable
