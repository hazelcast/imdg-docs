...
but only the caller of operation does updates synchronously after the modifying operation is done.
Except eviction and expiration, other modifying operations such as EntryProcessor, explicit update or remove, generate invalidations. 
For expiration and eviction, related configurations of Near Cache must be used.
