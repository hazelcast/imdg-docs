...
<<hazelcast-jca-resource-adapter, Hazelcast JCA Resource Adapter section>>
...
NOTE: To enable Spring integration, either `hazelcast-spring-{page-component-version}.jar`
or `hazelcast-all-{page-component-version}.jar` must be in the classpath.
...
http://static.springsource.org/spring/docs/3.1.x/spring-framework-reference/html/cache.html[Spring Cache Abstraction^].
...
transactions by using {docBaseUrl}/javadoc/com/hazelcast/spring/transaction/HazelcastTransactionManager.html[HazelcastTransactionManager^]
...
register {docBaseUrl}/javadoc/com/hazelcast/spring/transaction/ManagedTransactionalTaskContext.html[ManagedTransactionalTaskContext^]
