...
NOTE: When a task is deserialized, HazelcastInstance needs to be accessed. To do this, the task should implement `HazelcastInstanceAware` interface. See the <<implementing-hazelcastinstanceaware, HazelcastInstanceAware Interface section>> for more information.
...
* `statistics-enabled`: Specifies whether the statistics gathering is enabled for your Executor Service. If set to `false`, you cannot collect statistics in your implementation (using `getLocalExecutorStats()`) and also https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-executors[Hazelcast Management Center^] will not show them. Its default value is `true`.
...
IExecutorService can be configured to check for a minimum number of available members before applying its operations (see the <<split-brain-protection, Split-Brain Protection section>>). This is a check to avoid performing successful queue operations on all parts of a cluster during a network partition.
...
Split-brain protection for Executor Service can be configured programmatically using the method {docBaseUrl}/javadoc/com/hazelcast/config/ExecutorConfig.html[setSplitBrainProtectionName()^], or declaratively using the element `split-brain-protection-ref`. Following is an example declarative configuration:
...
The value of `split-brain-protection-ref` should be the split-brain protection configuration name which you configured under the `split-brain-protection` element as explained in the <<split-brain-protection, Split-Brain Protection section>>.
...
Durable Executor Service can be configured to check for a minimum number of available members before applying its operations (see the <<split-brain-protection, Split-Brain Protection section>>). This is a check to avoid performing successful queue operations on all parts of a cluster during a network partition.
...
Split-brain protection for Durable Executor Service can be configured programmatically using the method {docBaseUrl}/javadoc/com/hazelcast/config/DurableExecutorConfig.html[setSplitBrainProtectionName()^], or declaratively using the element `split-brain-protection-ref`. Following is an example declarative configuration:
...
See the {docBaseUrl}/javadoc/com/hazelcast/scheduledexecutor/IScheduledExecutorService.html[IScheduledExecutorService Javadoc^] for its API details.
...
IScheduledExecutorService can be configured to check for a minimum number of available members before applying its operations (see the <<split-brain-protection, Split-Brain Protection section>>). This is a check to avoid performing successful queue operations on all parts of a cluster during a network partition.
...
Split-brain protection for Scheduled Executor Service can be configured programmatically using the method {docBaseUrl}/javadoc/com/hazelcast/config/ScheduledExecutorConfig.html[setSplitBrainProtectionName()^], or declaratively using the element `split-brain-protection-ref`. Following is an example declarative configuration:
...
The {docBaseUrl}/javadoc/com/hazelcast/map/IMap.html[IMap interface^] provides the following methods for entry processing:
...
The method `getExecutorName()` method may also return two constants defined in the {docBaseUrl}/javadoc/com/hazelcast/core/Offloadable.html[Offloadable interface^]:
...
Remember to set the `pool-size` (count of executor threads per member) according to your execution needs. See the <<configuring-executor-service, Configuring Executor Service section>> for the configuration details.
