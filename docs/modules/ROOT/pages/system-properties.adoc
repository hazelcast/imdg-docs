...
along with this and the following diagnostic related system properties, see the xref:management:diagnostics.adoc[Diagnostics section].
...
|
|bool
| When it is set to `true`, IMDG does not check if the added dynamic
configuration conflicts with an existing dynamic configuration.
Existing dynamic configuration is not replaced. Note that, this property
is not meant to ignore the conflicts between static configuration and dynamic configuration.
If the added dynamic configuration exists in the static configuration of
the member, it will throw an exception regardless this property. 
...
| false
| bool
| Enable xref:management:jmx-monitoring.adoc[JMX] agent.
...
| false
| bool
| Notify xref:data-structures:map.adoc#listening-to-map-entries-with-predicates[entry listeners with predicates] on map entry updates with
...
Map Store configuration is `false`. See xref:data-structures:map.adoc#setting-write-behind-persistence[here] for the description of the `write-coalescing` element.
...
| true
| bool
| Enables buffering the collected xref:management:metrics.adoc[metrics] for Management Center if set to `true`, disables it otherwise. Note that
the preferred way for controlling this setting is xref:management:metrics.adoc#metrics-configuration[Metrics Configuration].

|`hazelcast.metrics.mc.retention`
| 5
| int
| Duration, in seconds, that the xref:management:metrics.adoc[metrics] are retained for Management Center. Note that
the preferred way for controlling this setting is xref:management:metrics.adoc#metrics-configuration[Metrics Configuration].
...
xref:clusters:advanced-network-configuration.adoc[advanced networking] is used.
...
This property should be considered with `hazelcast.shutdownhook.policy` whose
default value is `TERMINATE`; so when enabled (default behavior), this thread
