...
|count
|Remaining capacity on the executor's work queue

// TODO: Validate this file.partition metrics 
|`file.partition.freeSpace`
|bytes
|amoun of free space in the given directory, user.home
...
|count
|Total number of times the ID generator has been used to generate a new ID batch

|`flakeIdGenerator.creationTime`
|ms
|Creation time of this flakeIdGenerator on the member
...
|`gc.majorCount`
|count
|Total number of major garbage collections that have occurred

|`gc.majorTime`
|ms
|Accumulated elapsed time in major gc's 

|`gc.minorCount`
|count
|Total number of minor garbage collections that have occurred

|`gc.minorTime`
|ms
|Accumulated elapsed time in minor gc's 

|`gc.unknownCount`
|count
|Number of unknown garbage collections that cannot be determined to be as a minor or major (this is usually due to the lack of support of the used garbage collector)

|`gc.unknownTime`
|ms
|Accumulated elapsed time in unknown gc's

|`hot-restart.liveTombstones`
|count
|Number of live tombstones in the store

|`hot-restart.liveValues`
|count
|Number of live values in the store
...
|`map.index.hitCount`
|count
|Total number of hits into the index (the value of this metric may be greater than the `map.index.queryCount` since a single query may hit the same index more than once)
...
|count
|Number of times that the operation invocations timeout
...
|count
|Number of thrown exception on this TcpServerAcceptor
...
|count
|Number of connections establishments in progress
...
|ms
|Idle time that indicates how long since the last read/write
...
|count
|Number of times that io exceptions are thrown during selection
