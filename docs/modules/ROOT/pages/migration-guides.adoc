...
The following methods of {docBaseUrl}/javadoc/com/hazelcast/client/config/ClientConfig.html[ClientConfig^] have been refactored:
...
Also the `ClientAwsConfig` class has been renamed as {docBaseUrl}/javadoc/com/hazelcast/config/AwsConfig.html[AwsConfig^].
...
* {docBaseUrl}/javadoc/com/hazelcast/query/extractor/ValueCallback.html[ValueCallback^]
* {docBaseUrl}/javadoc/com/hazelcast/query/extractor/ValueExtractor.html[ValueExtractor^]
...
<<aggregations>> on top of Query infrastructure and the
...
See the {docBaseUrl}/javadoc/com/hazelcast/partition/MigrationListener.html[MigrationListener^] Javadoc
for a full insight.
...
See the following table for the before/after sample {docBaseUrl}/javadoc/com/hazelcast/security/IPermissionPolicy.html[IPermissionPolicy^] implementations.
...
The `getId()` method of the {docBaseUrl}/javadoc/com/hazelcast/nio/serialization/IdentifiedDataSerializable.html[IdentifiedDataSerializable^] interface
...
of {docBaseUrl}/javadoc/com/hazelcast/map/EntryProcessor.html[EntryProcessor^] which now defines how the entries will be processed
...
The {docBaseUrl}/javadoc/com/hazelcast/projection/Projection.html[Projection^] class was an abstract interface for historical reasons.
...
See the {docBaseUrl}/javadoc/com/hazelcast/config/ServiceConfig.html[here^]
...
See the {docBaseUrl}/javadoc/com/hazelcast/core/HazelcastInstance.html[here^]
...
See the {docBaseUrl}/javadoc/com/hazelcast/core/DistributedObjectEvent.html[here^]
...
By introducing the <<metrics, metrics system>> in Hazelcast IMDG 4.0,
the metrics collected by <<diagnostics, Diagnostics>>
...
since the data structure metrics are required for the other <<metric-consumers, Metric Consumers>>.
