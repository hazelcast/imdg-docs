...
Hazelcast Map uses the same eviction mechanism as the JCache implementation.
...
the eviction based on the expiration, can be configured using the element `time-to-live-seconds`
and `max-idle-seconds` as described above.
...
its size. The maximum allowed size can be configured using the element `size` and the
eviction policy can be configured using the element `eviction-policy` as described above.
...
as you can see in the interface {docBaseUrl}/javadoc/com/hazelcast/map/MapStore.html[code^].
...
see https://github.com/hazelcast/hazelcast-code-samples/tree/master/distributed-map/mapstore/src/main/java[here^].
...
{docBaseUrl}/javadoc/com/hazelcast/map/MapLoaderLifecycleSupport.html[`MapLoaderLifecycleSupport` interface^]
...
{docBaseUrl}/javadoc/com/hazelcast/config/MapStoreConfig.html[MapStoreConfig^]
...
{docBaseUrl}/javadoc/com/hazelcast/config/IndexConfig.html[IndexConfig^]
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#managing-maps[Hazelcast Management Center^].
...
See the <<listening-for-map-events, Listening for Map Events section>>.
...
information on how to add entry listeners to a map.
...
{docBaseUrl}/javadoc/com/hazelcast/map/MapInterceptor.html[`MapInterceptor` interface^]
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-queues[Hazelcast Management Center^]
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-multimaps[Hazelcast Management Center^]
will not show them. Its default value is `true`.
...
the method {docBaseUrl}/javadoc/com/hazelcast/config/MultiMapConfig.html[setSplitBrainProtectionName()^],
...
the method {docBaseUrl}/javadoc/com/hazelcast/config/SetConfig.html[setSplitBrainProtectionName()^],
...
the method {docBaseUrl}/javadoc/com/hazelcast/config/ListConfig.html[setSplitBrainProtectionName()^],
or declaratively using the element `split-brain-protection-ref`. Following is an example declarative configuration:
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-topics[Monitoring Topics section^]
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-topics[Hazelcast Management Center^]
will not show them. Its default value is `true`.
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-reliable-topics[Hazelcast Management Center^]
will not show them. Its default value is `true`.
...
If you execute the above `SemaphoreMember` class 5 times,
...
in `SemaphoreMember`, you will see that there is no limitation on the number
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-pn-counters[Hazelcast Management Center^]
will not show them. Its default value is `true`.
...
https://docs.hazelcast.org/docs/management-center/latest/manual/html/index.html#monitoring-replicated-maps[Hazelcast Management Center^]
will not show them. Its default value is `true`.
...
using the method {docBaseUrl}/javadoc/com/hazelcast/config/ReplicatedMapConfig.html[setSplitBrainProtectionName()^],
or declaratively using the element `split-brain-protection-ref`. Following is an example declarative configuration:
...
See the {docBaseUrl}/javadoc/com/hazelcast/cardinality/CardinalityEstimator.html[cardinality estimator Javadoc^]
...
programmatically using the method {docBaseUrl}/javadoc/com/hazelcast/config/CardinalityEstimatorConfig.html[setSplitBrainProtectionName()^],
...
{docBaseUrl}/javadoc/com/hazelcast/config/CardinalityEstimatorConfig.html[setMergePolicyConfig()^],
...
http://jet.hazelcast.org/[Hazelcast Jet^]. Because of this we describe
