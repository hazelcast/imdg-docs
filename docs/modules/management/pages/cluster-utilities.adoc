...
** You can make use of `PASSIVE` state along with the xref:storage:hot-restart-persistence.adoc[Hot Restart Persistence] feature. See the https://docs.hazelcast.org/docs/{page-component-display-version}/javadoc/com/hazelcast/core/Cluster.html#shutdown--[Cluster Shutdown API] for more info.
* **`IN_TRANSITION`**:
...
See the https://docs.hazelcast.org/docs/{page-component-display-version}/javadoc/com/hazelcast/core/Cluster.html[Cluster interface Javadoc] for information on these methods.
