...
using the REST API. See the {mc-docs}#hot-restart[Hot Restart^]
...
See the {mc-docs}#hot-restart[Hot Restart^]
...
or using the script `cluster.sh`. See the {mc-docs}#rolling-upgrade[Rolling Member Upgrades^]
and <<using-the-script-cluster-sh, `cluster.sh`>> sections.
|===
