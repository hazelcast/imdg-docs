...
. In the `antora.yml` file of the `.z` branch, increment the `version` field. For example, `4.1.2` becomes `4.1.3`.
...
. Create a `\{version\}.z` branch from the `master` branch, where `\{version\}` is the version of IMDG that you are releasing. For example, if you're working on content for version 5.0, find the branch named `4.0.z`.
+
TIP: For guidance on writing content, see the {url-contribute}[contribution guidelines].
...
. In the `antora.yml` file of the `.z` branch, increment the `version` field. For example, `5.0` becomes `5.0.1`.

. If you are releasing a new `latest-dev` version, in the `antora.yml` file of the `master` branch, increment the `version` field to the `latest-dev` version. For example, `5.0-SNAPSHOT` becomes `5.0.1-SNAPSHOT`.
...
/imdg/latest-dev/*  /imdg/4.2-snapshot/:splat 200!
----
